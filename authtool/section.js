		...
		function moveAtSections(pageX, pageY)
		{
			...
			{	
				...
				if(oRefElement)
				{
					...
					if(isInputValid(sElementId) && sElementId.search("HEADER_")!=-1 && (oRefElement.typeofsection==oSectionElement.sectiontype||oRefElement.typeofsection=="NOTECONTROL"&&oSectionElement.sectiontype!="NOTECONTROL")||(oRefElement.component=="section"&&oSectionElement.sectiontype=="SUBNOTECONTROL"))// && sElementId.search("row_")!=-1||sElementUse=="tablecontainer")
					{
						//debugger;
						//debugger;
